...
if len(version) > 0:
    if int(version) >= 0:
        ...
        use_floodfill = True
        backward_mode = 'random'
        print(f'PN Backward Mode: {backward_mode}')
        train_backward = True
        ...
        PN_option.train = True and not drop_PN
        DN_option.train = True

        # gradient mirror, last checkpoint
        # GN_option.weights = arch.Weights(l1=1.0*2*1.5/1.5*2, grad=1.0*0.5, image=1.0, net=1.0, null=1.0, gan=1.0/5/4, d_gan=1.0/5/4)
...
if PN_option.create:
    PN_generator = arch.PixelNet(input_channel=3, output_channel=3)
...
DN_buffer_x = arch.ImageBuffer()
DN_buffer_y = arch.ImageBuffer()
...
def train_mirror_image_loss(generated_image: torch.Tensor, target_image: torch.Tensor, option: arch.CombinedOptions):
    ...
    G_mirror_loss = arch.gradient_criterion(generated_image, target_image, option.weights.image)
...
for epoch in range(num_epochs):
    ...
    for i in range(n_total_steps):
        ...
        if not drop_PN:
            ...
            F_PN_output = F_GN_generated_image_256
        ...
        if GN_option.train:
            ...
            if GN_train_D_condition:
                pixel_grad_x, pixel_grad_y = arch.image_gradient_net(pixel_image)
                F_GN_generated_grad_256_x, F_GN_generated_grad_256_y = arch.image_gradient_net(F_GN_generated_image_256)
                GN_option.title = '256/X'
                GN_D_loss_256_x = train_discriminator(GN_discriminator, GN_buffer_256_x, F_GN_generated_grad_256_x, pixel_grad_x, GN_option)
                GN_option.title = '256/Y'
                GN_D_loss_256_y = train_discriminator(GN_discriminator, GN_buffer_256_y, F_GN_generated_grad_256_y, pixel_grad_y, GN_option)
                loss_buffer.GN_D_loss = (GN_D_loss_256_x + GN_D_loss_256_y) * 0.5

            # Training Generator
            if GN_train_G_condition:
                F_GN_generated_grad_256_x, F_GN_generated_grad_256_y = arch.image_gradient_net(F_GN_generated_image_256)
                GN_option.title = '256/'
                GN_G_loss_gan_256_x = train_adversarial_loss(GN_discriminator, F_GN_generated_grad_256_x, GN_option)
                GN_G_loss_gan_256_y = train_adversarial_loss(GN_discriminator, F_GN_generated_grad_256_y, GN_option)
                GN_G_loss_gan_256 = (GN_G_loss_gan_256_x + GN_G_loss_gan_256_y) * 0.5
                ...
                # if backward_mode == 'all':
                #     GN_G_loss_mirror_image = GN_G_loss_mirror_image_64 + GN_G_loss_mirror_image_48 + GN_G_loss_mirror_image_32
                # else:
                #     GN_G_loss_mirror_image = data.choose_random((GN_G_loss_mirror_image_64, GN_G_loss_mirror_image_48, GN_G_loss_mirror_image_32))
                GN_G_loss_mirror_image = GN_G_loss_mirror_image_256
                loss_buffer.GN_G_loss = GN_G_loss_256 + GN_G_loss_mirror_image
        ...
        if DN_option.train:
            ...
            if DN_train_D_condition:
                cartoon_grad_x, cartoon_grad_y = arch.image_gradient_net(cartoon_image)
                B_DN_generated_grad_x, B_DN_generated_grad_y = arch.image_gradient_net(B_DN_generated_image)
                DN_option.title = 'X'
                DN_D_loss_x = train_discriminator(DN_discriminator, DN_buffer_x, B_DN_generated_grad_x, cartoon_grad_x, DN_option)
                DN_option.title = 'Y'
                DN_D_loss_y = train_discriminator(DN_discriminator, DN_buffer_y, B_DN_generated_grad_y, cartoon_grad_y, DN_option)
                loss_buffer.DN_D_loss = (DN_D_loss_x + DN_D_loss_y) * 0.5
            ...
            if DN_train_G_condition:
                ...
                DN_option.title = ''
                DN_G_loss_gan_x = train_adversarial_loss(DN_discriminator, B_DN_generated_grad_x, DN_option)
                DN_G_loss_gan_y = train_adversarial_loss(DN_discriminator, B_DN_generated_grad_y, DN_option)
                DN_G_loss_gan = (DN_G_loss_gan_x + DN_G_loss_gan_y) * 0.5
                ...
                loss_buffer.DN_G_loss = DN_G_loss_gan + DN_G_loss_l1 + DN_G_loss_grad + DN_G_loss_mirror_image
        ...
        if use_tensorboard and saveCondition:
            ...
            writer.add_images(prefix + 'MIRROR/256', B_GN_generated_image_256 / 2 + 0.5, current_iter)
